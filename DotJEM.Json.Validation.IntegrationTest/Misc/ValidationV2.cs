...
public class ValidationV2ConstraintBuilder
{
    ...
    [Test]
    public void EmptyValidator_EmptyValue_ReturnsFailure()
    {
        EmptyValidator validator = new EmptyValidator();
        ValidatorResult result = validator.Validate(new JObject(), new JsonValidationContext());
        Assert.That(result.IsValid, Is.False);
...
public class EmptyValidator : JsonValidator
{
    public EmptyValidator()
    {
        When(Any).Then(new SelfSelector(), Must.Have.Value());
    }

    public class SelfSelector : FieldSelector
    {
        public override string Alias { get; } = ".";
        public override IEnumerable<JTokenInfo> SelectTokens(JObject entity)
        {
            yield return new JTokenInfo(entity, this);
        }
    }
}
